...
try {
    let qw = fs.readFileSync('/app/applet/queue-worker.js', 'utf8');
    
    const targetBlock = `                if (options?.templateName) {
    ...
    const replacement = `                if (options?.templateName) {
                    msgData.type = "template";
                    msgData.template = {
                        name: options.templateName,
                        language: { code: options.templateLanguage || 'en' }
    ...
    if (qw.includes(targetBlock)) {
        ...
        fs.writeFileSync('/app/applet/queue-worker.js', qw);
        console.log("Patched queue-worker.js successfully.");
    } else {
        console.log("Could not find target block in queue-worker.js");
    }
