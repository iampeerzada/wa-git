#!/bin/bash
# Patch for fixing Meta template creation with media header
...
try {
    ...
    // 2. Patch Templates.tsx
    let ui = fs.readFileSync('/var/www/wa-api/dist/assets/index.js', 'utf8');
    if (ui.includes('header_handle:[')) {
        // UI build modification is complex. Let's patch source and rebuild
    }
} catch (e) {
