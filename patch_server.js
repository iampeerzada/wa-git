...
try {
    ...
    const replacement = `        // Intercept components to handle media uploads for examples
    ...
    if (srv.includes(target)) {
        ...
        console.log("Patched server.cjs for POST /create");
    } else {
        console.log("Could not find POST target");
    }
    ...
    if (srv.includes(targetEdit)) {
        ...
        fs.writeFileSync('server.cjs', srv);
        console.log("Patched server.cjs for POST /edit");
    } else {
        console.log("Could not find EDIT target");
    }
