            ...
            <form onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              {formData.action_type === 'message' && (
                    <div className="md:col-span-2">
                      ...
                      <textarea required className="w-full bg-[#202c33] border border-gray-600 p-3 rounded-lg text-white outline-none min-h-[140px]" value={formData.text_content} onChange={(e: any) => setFormData({...formData, text_content: e.target.value})} placeholder={"Welcome to iFastX\n\nPlease select an option:\n1. Billing\n2. Support"}></textarea>
                      <p className="text-xs text-gray-500 mt-2">Write the text exactly as it will appear on WhatsApp.</p>
                      
                      <label className="block text-sm font-medium mb-1.5 text-gray-300 mt-4">Media URL (Optional)</label>
                      <div className="flex gap-2 relative">
                          <input type="url" placeholder="https://example.com/image.jpg" className="w-full bg-[#202c33] border border-gray-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none p-2.5 rounded-lg text-white transition-all" value={formData.media_url || ''} onChange={(e: any) => setFormData({...formData, media_url: e.target.value})} />
